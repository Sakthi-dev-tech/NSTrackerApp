...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    double tonnerMove = MediaQuery.of(context).size.width *
            0.9 *
            0.8 *
            context.watch<HomepageProvider>().progressValue -
        10;
    return Scaffold(
                        ...
                        child: Row(
                          ...
                          children: [
                            InformationWidgets(
                              ...
                              data:
                                  context.watch<HomepageProvider>().ipptDisplay,
                            ),
                            InformationWidgets(
  ...
  context.read<HomepageProvider>().updateHomepage(
    newLeavesLeft: homepageBox.get('leavesLeft'),
    newIPPTDisplay: homepageBox.get('ipptResult'),
    payDay: payDayToDropdownValue[homepageBox.get('payDay')]!,
    enlistmentDate: homepageBox.get('enlistmentDate'),
    ordDate: homepageBox.get('ordDate'),
    );

    setState(() {
      dateForUpcomingHoliday = nextUpcomingHolidayDate();
      numberOfDaysToUpcomingHoliday = DateTime.parse(dateForUpcomingHoliday).difference(DateTime.now()).inDays + 1;
    });
  }
}
